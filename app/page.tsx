...
export default function Home(){
  ...
  useEffect(()=>{ fetch("/api/portfolio").then(r=>r.json()).then(d=> setLinks(d.links||[])); },[]);
    ...
    <div className="min-h-screen">
      ...
      <section id="portfolio" className="py-16 bg-white">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="text-neutral-600 mt-1 text-sm">Admin 可在 /admin 添加 Redfin/Zillow 链接。</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
            {links.map(l => (
              <Card key={l.id}>
                <CardHeader>
                  <CardTitle className="text-base">{l.title || "Listing"}</CardTitle>
                  <CardDescription className="truncate">{l.url}</CardDescription>
                </CardHeader>
                <CardFooter>
                  <a href={l.url} target="_blank" className="inline-flex items-center gap-2 text-sm text-blue-600"><Link2 className="w-4 h-4"/> 打开</a>
                </CardFooter>
              </Card>
            ))}
            {links.length===0 && <div className="text-sm opacity-70">暂时还没有案例链接。</div>}
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="py-16">
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-semibold">告诉我们你的项目</h2>
            <ContactForm/>
          </div>
...
function ContactForm(){
  ...
  return (
    <form className="mt-4 grid gap-3" onSubmit={async (e)=>{e.preventDefault(); const r=await fetch("/api/messages",{method:"POST",headers:{'content-type':'application/json'},body:JSON.stringify({name,email,phone,content})}); if(r.ok){ setOk(true); setName(''); setEmail(''); setPhone(''); setContent(''); }}}>
      <Input placeholder="姓名" value={name} onChange={e=>setName((e.target as any).value)} required/>
      <Input type="email" placeholder="邮箱" value={email} onChange={e=>setEmail((e.target as any).value)} required/>
      <Input placeholder="电话（可选）" value={phone} onChange={e=>setPhone((e.target as any).value)}/>
      <Textarea placeholder="备注（风格/预算/时间点）" value={content} onChange={e=>setContent((e.target as any).value)} rows={5}/>
      <Button>发送</Button>
