...
function Gate({ onOk }:{ onOk:(code:string)=>void }){
  ...
  return (<div className="min-h-[60vh] grid place-items-center">
    <Card className="w-full max-w-sm">
      ...
      <CardContent><Input type="password" placeholder="Access code" value={code} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setCode(e.target.value)}/></CardContent>
...
function AdminInbox(){
  ...
  useEffect(()=>{(async()=>{try{const r=await fetch('/api/messages'); if(!r.ok) throw new Error(await r.text()); const d=await r.json(); setList(d.messages||[]);}catch(e){console.error('Messages load error',e); setList([]);}})();},[]);
...
function AdminPortfolio({ adminCode }:{ adminCode:string }){
  ...
  async function reload(){ try{const r=await fetch('/api/portfolio'); if(!r.ok) throw new Error(await r.text()); const d=await r.json(); setLinks(d.links||[]);}catch(e){console.error('Portfolio load error',e); setLinks([]);} }
  useEffect(()=>{ reload(); },[]);
  return (<div className="space-y-4">
    <Card><CardHeader><CardTitle>添加 Redfin/Zillow 链接</CardTitle><CardDescription>粘贴房源链接，可附加标题/备注。</CardDescription></CardHeader>
      <CardContent className="grid md:grid-cols-3 gap-2">
        <Input placeholder="URL (Redfin/Zillow)" value={url} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setUrl(e.target.value)}/>
        <Input placeholder="Title (optional)" value={title} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setTitle(e.target.value)}/>
        <Input placeholder="Note (optional)" value={note} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNote(e.target.value)}/>
      </CardContent>
...
function AdminInventory({ adminCode }:{ adminCode:string }){
  ...
  async function reload(){ try{ const r=await fetch('/api/items'); if(!r.ok) throw new Error(await r.text()); const d=await r.json(); setItems(d.items||[]);} catch(e){ console.error('Items load error',e); setItems([]);} }
  useEffect(()=>{ reload(); },[]);
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><Warehouse className="w-5 h-5"/> Inventory</CardTitle></CardHeader>
      <CardContent>
        <div className="grid md:grid-cols-6 gap-2">
          <Input placeholder="Name" value={newItem.name} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNewItem({...newItem,name:e.target.value})}/>
          <select className="border rounded-lg px-3 py-2 text-sm" value={newItem.category} onChange={(e: React.ChangeEvent<HTMLSelectElement>)=>setNewItem({...newItem,category:e.target.value})}>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select>
          <select className="border rounded-lg px-3 py-2 text-sm" value={newItem.color} onChange={(e: React.ChangeEvent<HTMLSelectElement>)=>setNewItem({...newItem,color:e.target.value})}>{COLORS.map(c=><option key={c}>{c}</option>)}</select>
          <Input placeholder="Size" value={newItem.size} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNewItem({...newItem,size:e.target.value})}/>
          <Input type="number" placeholder="$/month" value={newItem.price} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNewItem({...newItem,price:Number(e.target.value)})}/>
          <Input type="date" placeholder="Inbound" value={newItem.inbound} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNewItem({...newItem,inbound:e.target.value})}/>
          <Input placeholder="SKU (auto optional)" value={newItem.sku} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNewItem({...newItem,sku:e.target.value})}/>
        </div>
    ...
    <Card><CardHeader><CardTitle>Inventory List</CardTitle></CardHeader>
      <CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left border-b"><th className="py-2">SKU</th><th>Name</th><th>Cat</th><th>Color</th><th>Size</th><th>$/mo</th><th>Status</th><th className="text-right">Actions</th></tr></thead>
        <tbody>{items.map(it=>(<tr key={it.id} className="border-b"><td className="py-2 font-mono">{it.sku}</td><td>{it.name}</td><td>{it.category}</td><td>{it.color}</td><td>{it.size}</td><td>{it.price}</td>
          ...
          <td className="text-right"><Button variant="outline" size="sm" onClick={()=>toggle(it)}>{it.status==='in'?'Check-out':'Check-in'}</Button></td></tr>))}</tbody></table></div></CardContent>
    </Card>
  </div>);
}
