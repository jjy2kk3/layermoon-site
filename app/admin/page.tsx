    ...
    <div className="min-h-[60vh] grid place-items-center">
      <Card className="w-full max-w-sm">
        <CardHeader>
          ...
          <CardDescription>输入访问码进入后台</CardDescription>
        </CardHeader>
        <CardContent>
          <Input
            type="password"
        ...
        <CardFooter>
          <Button className="w-full" onClick={() => onOk(code)}>
...
export default function Admin() {
  ...
  const [tab, setTab] = useState<"inventory" | "portfolio" | "inbox">(
    "inventory",
  );
  return (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Contact 表单提交</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          ...
          {list.length === 0 && (
            <div className="text-sm opacity-70">暂无消息。</div>
          )}
        </div>
      </CardContent>
    </Card>
...
function AdminPortfolio({ adminCode }: { adminCode: string }) {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle>添加 Redfin/Zillow 链接</CardTitle>
          <CardDescription>粘贴房源链接，可附加标题/备注。</CardDescription>
        </CardHeader>
        <CardContent className="grid md:grid-cols-3 gap-2">
          <Input
            placeholder="URL (Redfin/Zillow)"
            value={url}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
          ...
          <Card key={l.id}>
            ...
            <CardFooter className="flex justify-between">
              <a
                href={l.url}
                target="_blank"
                className="text-blue-600 text-sm inline-flex items-center gap-2"
              >
              ...
              <Button
                ...
                size="sm"
                onClick={async () => {
                  await fetch(`/api/portfolio?id=${l.id}`, {
                    method: "DELETE",
...
function AdminInventory({ adminCode }: { adminCode: string }) {
  ...
  async function add() {
    ...
    const sku =
      newItem.sku ||
      skuFromPattern(newItem.color, newItem.category, newItem.inbound, seq);
        ...
        <CardContent>
          <div className="grid md:grid-cols-6 gap-2">
            ...
            <Input
              type="date"
              placeholder="Inbound"
              value={newItem.inbound}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                setNewItem({ ...newItem, inbound: e.target.value })
              }
            />
            <Input
              placeholder="SKU (auto optional)"
              value={newItem.sku}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  ...
                  <tr key={it.id} className="border-b">
                    ...
                    <td className="text-right">
                      <Button
                        ...
                        onClick={() => toggle(it)}
                      >
                        {it.status === "in" ? "Check-out" : "Check-in"}
                      </Button>
                    </td>
                  </tr>
                ))}
